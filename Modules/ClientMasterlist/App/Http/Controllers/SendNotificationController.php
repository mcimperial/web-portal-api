...
class SendNotificationController extends Controller
{
    ...
    private function sendSingleEmail($data, $notification)
    {
        ...
        // MULTI-PROVIDER FEATURE: REPORT: ATTACHMENT (APPROVED)
        // Mirrors SUBMITTED's structure but:
        //   - Uses generateMultiProviderCsvAttachments() (one XLSX per provider)
        //   - Sends a placeholder email when no enrollees are found (instead of silently skipping)
        else if ($notification->notification_type === 'REPORT: ATTACHMENT (APPROVED)' && !$placeholderMessage) {
            // If a single csv_attachment was pre-generated by sendScheduled, wrap it for consistent handling
            if ($csvAttachment && is_array($csvAttachment) && isset($csvAttachment['has_data']) && $csvAttachment['has_data']) {
                ...
                // Generate fresh multi-provider XLSXs for manual / direct sends
                ...
                if (is_array($statusResult) && isset($statusResult['type']) && $statusResult['type'] === 'csv_generation') {
                    ...
                    if (empty($csvAttachments)) {
                        ...
                        $placeholderMessage = "<b>Hi {$companyName},</b> <br /><br />"
                            . "This is to notify that there are no new active members as of today, " . date('F j, Y') . ".<br /><br />"
        ...
        else if ($notification->notification_type === 'REPORT: ATTACHMENT (SUBMITTED)' && !$placeholderMessage) {
            // If csv_attachment was pre-generated by sendScheduled, wrap it for consistent handling
            if ($csvAttachment && is_array($csvAttachment) && isset($csvAttachment['has_data']) && $csvAttachment['has_data']) {
                $csvAttachments = [$csvAttachment];
            } else if (!$csvAttachment) {
                // Generate fresh for manual/direct sends
                $statusResult = $this->checkNotificationStatus($notification->notification_type, $notification->enrollment_id ?? null, $notification);
    ...
    private function checkNotificationStatus($notificationType, $enrollmentId = null, $notification = null)
    {
        ...
        switch ($notificationType) {
            ...
            case 'WARNING NOTIFICATION: ENROLLEE':
                return $this->getEnrolleesByStatus($enrollmentId, 'NC', ['PENDING', 'SUBMITTED-PERSONAL-INFORMATION'], $notification, 3);
     ...
     * each as a plain XLSX file (no password, no ZIP — to be added later).
     *
     * LOGIC:
